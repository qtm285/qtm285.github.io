...
(function() {
  ...
  // The toggle frames: direct children of the cells wrapper that hold one figure each,
  // either a div wrapping a .cell or a bare .cell.
  function findFrames(cellsWrapper) {
    const frames = [];
    for (const child of Array.from(cellsWrapper.children)) {
  ...
  // Ready means at least two frames have a plot in them. Figures baked at render
  // time are ready at DOMContentLoaded; WebR figures are empty until webR boots
  // and runs, which takes minutes.
  function isReady(container) {
    if (container.querySelectorAll('img, svg').length < 2) return false;
    const cellsWrapper = container.querySelector('[aria-describedby]');
    if (!cellsWrapper) return false;
    return findFrames(cellsWrapper).filter(f => f.querySelector('img, svg')).length >= 2;
  }

  // Initialize when the plots arrive. One observer per container, disconnected
  // the moment it fires, so a container is never initialized twice.
  function watchForImages(container) {
    if (watched.has(container)) return;
    // The figure wrapper is baked at render time. A container without one — a
    // .image-toggle with no id, so no caption — will never grow one, and
    // initImageToggle would bail on it anyway. Nothing to wait for.
    if (!container.querySelector('[aria-describedby]')) return;
    watched.add(container);
    const observer = new MutationObserver(() => {
      if (!isReady(container)) return;
      observer.disconnect();
      initImageToggle(container);
    });
  ...
  function initImageToggle(container) {
    ...
    if (!isReady(container)) { watchForImages(container); return; }
    ...
    const cells = findFrames(cellsWrapper);
